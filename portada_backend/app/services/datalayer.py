...
class DataLayerService:
    ...
    def __init__(self):
        ...
        if env_config_path:
            ...
            default_path = "/data/config/delta_data_layer_config.json"
            local_fallback = (
            ...
            self.config_path = (
                str(local_fallback) if local_fallback.exists() else default_path
            )
    ...
    def _init_layers(self):
        ...
        if os.path.exists(self.config_path):
            ...
            logger.warning(
                f"Config file not found at {self.config_path}. Using empty config."
            )
            self.config = {}
        ...
        self.entities_layer = self.data_layer_builder.build(
            PortadaBuilder.KNOWN_ENTITIES_TYPE
        )
        ...
        # Metadata layer
        self.metadata_layer = DataLakeMetadataManager(
            self.boat_layer.get_configuration()
        )
    ...
    def get_missing_dates(
        ...
        if date_list:
            ...
            result = self.boat_layer.get_missing_dates_from_a_newspaper(
                publication_name=publication_name, date_and_edition_list=date_list
            )
        else:
    ...
    def get_duplicate_metadata(
        self, publication=None, user=None, start_date=None, end_date=None
    ):
        """
        ...
        try:
            ...
            if publication:
                df_dup = df_dup.filter(
                    F.lower(F.col("publication")) == publication.lower()
                )
            if user:
    ...
    def count_entries(self, publication, start_date=None, end_date=None):
        ...
        df = (
            df.withColumn("data_dt", F.to_date("publication_date", "yyyy-MM-dd"))
        ...
        df_counter = (
            df.rollup("y", "m", "d", "publication_edition")
        ...
        for row in resultados:
            if row.y is None:
                ...
                if year not in years:
                    years[year] = {"year": year, "count": row["count"], "months": {}}
            elif row.d is None:
                # Total por mes
                year = row.y
                month = row.m
                ...
                years[year]["months"][month]["days"][day]["editions"].append(
                    {"edition": edition, "count": row["count"]}
                )

        # Convertir diccionarios a listas
        result = {"total": total, "years": []}
    ...
    def list_known_entities(self):
        ...
        entities_path = os.path.join(
            base_path, project_name, "ingest", "known_entities"
        )
        ...
        if os.path.exists(entities_path):
            for item in os.listdir(entities_path):
                ...
                if os.path.isdir(path):
                    ...
                    entities.append({"name": item, "type": item, "count": count})
    ...
    def get_publications(self):
        ...
        ship_entries_path = os.path.join(
            base_path, project_name, "ingest", "ship_entries"
        )
        pubs = set()
